...
define('DISCUZ_ROOT', LIB_PATH."Uc/");
...
class uc_note {
	...
	function synlogin($get, $post) {
		...
		$vo = M("member")->field("id,user_name")->where("user_name='{$username}'")->find();
		if(is_array($vo)){//登陆
			foreach($vo as $key=>$v){
				session("u_{$key}",$v);
			}
			...
			$count = M('member')->where("user_email = '{$data['user_email']}' OR user_name='{$data['user_name']}'")->count('id');
			if($count>0) exit;;
			...
			if($newid){
				...
				Notice(1,$newid);
				logMoney($newid,1,$this->glo['award_reg'],"注册奖励");
			}
		}
