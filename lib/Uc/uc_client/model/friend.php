...
class friendmodel {
	...
	function get_list($uid, $page, $pagesize, $totalnum, $direction = 0) {
		...
		if($sqladd) {
			$data = $this->db->fetch_all("SELECT f.*, m.username FROM ".UC_DBTABLEPRE."friends f LEFT JOIN ".UC_DBTABLEPRE."member m ON f.friendid=m.uid WHERE $sqladd LIMIT $start, $pagesize");
			return $data;
		} else {
			return array();
		}
