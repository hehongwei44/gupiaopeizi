...
class mailmodel {
	...
	function get_list($page, $ppp, $totalnum) {
		...
		$data = $this->db->fetch_all("SELECT m.*, u.username, u.email FROM ".UC_DBTABLEPRE."mailqueue m LEFT JOIN ".UC_DBTABLEPRE."member u ON m.touid=u.uid ORDER BY dateline DESC LIMIT $start, $ppp");
	...
	function add($mail) {
		if($mail['level']) {
			...
			$users = $this->db->fetch_all("SELECT uid, username, email FROM ".UC_DBTABLEPRE."member WHERE uid IN ($uids)");
			foreach($users as $v) {
	...
	function _get_mail() {
		$data = $this->db->fetch_first("SELECT m.*, u.username, u.email FROM ".UC_DBTABLEPRE."mailqueue m LEFT JOIN ".UC_DBTABLEPRE."member u ON m.touid=u.uid WHERE failures<'".UC_MAIL_REPEAT."' ORDER BY level DESC, mailid ASC LIMIT 1");
		return $data;
	}

	function _get_mail_by_id($mailid) {
		$data = $this->db->fetch_first("SELECT m.*, u.username, u.email FROM ".UC_DBTABLEPRE."mailqueue m LEFT JOIN ".UC_DBTABLEPRE."member u ON m.touid=u.uid WHERE mailid='$mailid'");
		return $data;
	}
