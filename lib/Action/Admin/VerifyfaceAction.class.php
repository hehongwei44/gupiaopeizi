...
class VerifyFaceAction extends AdminAction
{
    /**
    +----------------------------------------------------------
    * 默认操作
    ...
    */
    public function index()
    {
		...
		$count = M('apply_face v')->join("{$this->pre}member m ON m.id=v.uid")->join("{$this->pre}member_account mm ON mm.uid=m.id")->join("{$this->pre}member_info mi ON mi.uid=m.id")->where($map)->count('m.id');
		...
		$list = M('apply_face v')->field($field)->join("{$this->pre}member m ON m.id=v.uid")->join("{$this->pre}member_account mm ON mm.uid=m.id")->join("{$this->pre}member_info mi ON mi.uid=m.id")->where($map)->limit($Lsql)->order('v.id DESC')->select();
	...
	public function doEdit(){
        ...
        if ($result = $model->save()) { //保存成功
			...
			$uid = M('apply_face')->getFieldById($_POST['id'],'uid');

			if($_POST['apply_status'] == 1){
                setMemberStatus($uid, 'face', $_POST['apply_status'], 8, '现场');   
				sendMessage($uid,"您的现场认证审核通过","您的现场认证审核通过");
				alogs("VerifyFace",0,1,'现场认证审核通过！');//管理员操作日志
			}else{
				sendMessage($uid,"您的现场认证审核未通过","您的现场认证审核未通过");
