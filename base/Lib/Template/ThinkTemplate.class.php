...
class  ThinkTemplate {
    ...
    protected function parseIncludeItem($tmplPublicName,$vars=array()){
        ...
        if(false === strpos($tmplPublicName,$this->config['template_suffix'])) {
            ...
            if(!empty($path)) {
				if(isset($vars['group'])){
					$tpathe = explode("/",dirname(THEME_PATH));array_pop($tpathe);
					$path = implode("/",$tpathe).'/'.$vars['group'].'/'.array_pop($path).'/';
				}else{
        ...
        }elseif(true===$GLOBALS['is_iis']){
			if(realpath($tmplPublicName)) $tmplPublicName = realpath($tmplPublicName);
			else $tmplPublicName = substr(C('WEB_ROOT'),0,-2)."/".$tmplPublicName;
		}
		
		$tmplPublicName = realpath($tmplPublicName);
        // 获取模板文件内容
