...
class Image {
    ...
    */
    static function buildImageVerify($length=4, $mode=3, $type='png', $width=105, $height=36, $size='24',$verifyName='verify',$leftspace=10) {
        $fontpath=  dirname(__PATH__).'/res/font/calibri.ttf';
        import('ORG.Util.String');
        $randval = String::randString($length, $mode);
        ...
        if(file_exists($fontpath)){
            for ($i = 0; $i < $length; $i++) {
                $x = $size * $i + $left;
                imagettftext($im, $size, mt_rand(-30, 35), $x, $y, $whites, $fontpath, $randval{$i});
            }
        }else{
            for ($i = 0; $i < $length; $i++) {
